...
function begin(environments) {
    ...
    var testQueue = captures.map(function(captureObj) {
        return function() {
            return new Promise(function(resolve) {
                ...
                var multiremote = getMultiremoteConfig(environments,
                                                       captureName);

                console.log('Investigating ' + captureName + '...');

                var tryCapture = function(client) {
                    // Wrap capture call to catch user Webdriver errors.
                    return new Promise(function(resolve) {
                        try {
                            resolve(capture(client));
                        } catch (e) {
                            console.log(e);
                            reject();
                        }
                ...
                var sessions;
                tryCapture(client).then(function(client) {
    ...
    series.run().then(function() {
        ...
        var doneUrl = urljoin(API_ROOT, 'builds',
...
function getMultiremoteConfig(environments, captureName) {
    ...
    environments.forEach(function(browserEnv, i) {
        ...
        if (captureName) {
            multiremote['browser' + i].desiredCapabilities.name =
                'Sherlocked: ' + captureName;
        }
    });
...
function uploadCapture(image, browserEnv, captureName, sauceSessionId) {
    ...
    var captureUrl = urljoin(API_ROOT, 'builds',
                             process.env.TRAVIS_BUILD_ID.toString(),
                             'captures/');
