...
describe('generateClient', function() {
    ...
    it('generates a client', function() {
        var multiConfig = sherlocked.getMultiremoteConfig(getEnvironments());
...
describe('getMultiremoteConfig', function() {
    var getMultiremoteConfig = sherlocked.getMultiremoteConfig;

    it('builds a multi-browser config', function() {
        var environments = getEnvironments();
        environments.push({browserName: 'chrome'});
        var multiConfig = getMultiremoteConfig(environments);
    ...
    it('attaches Sauce credentials', function() {
        var multiConfig = getMultiremoteConfig(getEnvironments());
    ...
    it('attaches capture name', function() {
        var multiConfig = getMultiremoteConfig(getEnvironments(), 'Home Page');
        assert.equal(multiConfig.browser0.desiredCapabilities.name,
                     'Sherlocked: Home Page');
    });
});
...
describe('investigate', function() {
    ...
    it('chains investigate', function() {
        assert.ok(
    ...
    it('chains begin', function() {
        assert.ok(investigate('Test Capture', function() {}).begin);
    });
});
...
describe('begin', function() {
    it('starts', function() {
        ...
        assert.ok(sherlocked.begin(getEnvironments()));
    });
});
